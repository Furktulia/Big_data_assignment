...
object FPFunctions {
    ...
    /** Q18 (5p)
      ...
      * Prohibitions: do not use library functions such as `reverse` or `foldRight`.
      * @param xs the list to fold.
      * @param f the fold function.
      * @param init the initial value.
      * @tparam A the type of the items in `xs`.
    ...
    def foldR[A, B](xs: List[A], f: (A, B) => B, init: B): B = {
        val foldFunction = (g: B => B, x: A) => (b: B) => g(f(x, b))

        val identityFunction = (b: B) => b
        foldL(xs, foldFunction, identityFunction)(init)
    }
